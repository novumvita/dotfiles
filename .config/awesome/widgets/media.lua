...
local worker = function(user_args)
	...
	local color = user_args.color or "#ffffff"
	...
	local update_widget = function(widget, stdout, stderr)
		...
		for p in string.gmatch(escaped, "([^\n]*)\n") do
			...
			local song_markup = string.format("<span font='%s' foreground='%s'>%s</span>", font, player == "spotify" and "#1db954" or color, song_text);
